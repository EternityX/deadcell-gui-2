...
//  2021/01/26: simplified integration by using '#define IMGUI_ENABLE_FREETYPE'. renamed ImGuiFreeType::XXX flags to
//  ImGuiFreeTypeBuilderFlags_XXX for consistency with other API. removed ImGuiFreeType::BuildFontAtlas(). 2020/06/04: fix for
//  rare case where FT_Get_Char_Index() succeed but FT_Load_Glyph() fails. 2019/02/09: added RasterizerFlags::Monochrome flag to
//  disable font anti-aliasing (combine with ::MonoHinting for best results!) 2019/01/15: added support for imgui allocators +
//  added FreeType only override function SetAllocatorFunctions(). 2019/01/10: re-factored to match big update in STB builder.
//  fixed texture height waste. fixed redundant glyphs when merging. support for glyph padding. 2018/06/08: added support for
//  ImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX. 2018/02/04: moved to main imgui repository (away from
//  http://www.github.com/ocornut/imgui_club) 2018/01/22: fix for addition of ImFontAtlas::TexUvscale member. 2017/10/22: minor
//  inconsequential change to match change in master (removed an unnecessary statement). 2017/09/26: fixes for imgui internal
//  changes. 2017/08/26: cleanup, optimizations, support for ImFontConfig::RasterizerFlags, ImFontConfig::RasterizerMultiply.
//  2017/08/16: imported from https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated for
//  latest changes in ImFontAtlas, minor tweaks.
...
namespace {
    ...
    struct FontInfo {
        ...
        float LineSpacing;  // The baseline-to-baseline distance. Note that it usually is larger than the sum of the ascender and
                            ...
                            // subsequent baselines when using this distance. Think of it as a value the designer of the font
                            // finds appropriate.
                ...
                if (multiply_table == nullptr) {
                    ...
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch) {
                        for (uint32_t x = 0; x < w; x++) {
                            uint8_t r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4], a = src[x * 4 + 3];
                            dst[x] = IM_COL32(
                                multiply_table[DE_MULTIPLY(r, a)], multiply_table[DE_MULTIPLY(g, a)],
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas *atlas, unsigned int extra_flags) {
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++) {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++) {
            ...
            IM_ASSERT(
                buf_bitmap_current_used_bytes + bitmap_size_in_bytes <= BITMAP_BUFFERS_CHUNK_SIZE
            );  // We could probably allocate custom-sized buffer instead.

            // Blit rasterized pixels to our temporary buffer and keep a pointer to it.
            src_glyph.BitmapData = (unsigned int *) (buf_bitmap_buffers.back() + buf_bitmap_current_used_bytes);
            buf_bitmap_current_used_bytes += bitmap_size_in_bytes;
            src_tmp.Font.BlitGlyph(
                ft_bitmap, src_glyph.BitmapData, src_glyph.Info.Width, multiply_enabled ? multiply_table : nullptr
            );
    ...
    stbrp_init_target(
        &pack_context, atlas->TexWidth - atlas->TexGlyphPadding, TEX_HEIGHT_MAX - atlas->TexGlyphPadding, pack_nodes.Data,
        pack_nodes.Size
    );
